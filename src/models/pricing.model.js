export async function findByRecipe(conn, recipeId) {
  const [rows] = await conn.query(
    `SELECT id, recipe_id, hpp_per_unit, target_margin_percent, suggested_price, updated_at
...
export async function upsert(conn, recipeId, { hpp_per_unit, target_margin_percent, suggested_price }) {
  const [result] = await conn.query(
    `INSERT INTO pricing (recipe_id, hpp_per_unit, target_margin_percent, suggested_price)
     VALUES (?, ?, ?, ?)
    ...
    [recipeId, hpp_per_unit, target_margin_percent, suggested_price]
  );
  return result;
}
